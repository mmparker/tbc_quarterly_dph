...
ggplot(actives_count, aes(x = plot_qtr, y = freq, group = 1)) +
    ...
    labs(x = "Quarter identified (earliest of date treatment started or date case reported)", 
         y = "Number of cases",
         title = "TB Cases in Metropolitan Denver, by Quarter") +
...
ggplot(metroactives, aes(x = yr_id, fill = plot_group)) +
    ...
    labs(x = "Quarter Identified (earliest of date treatment started or date case reported)", 
         y = "Number of cases",
         title = "TB Cases by County of Residence") +
    theme_tb + theme(axis.text.x = element_text(angle = 70, 
                                                hjust = 1.30, 
                                                vjust = 1.3))
...
nat_melt$variable[nat_melt$variable %in% "national_average"] <- "National Average (Standard Cases)"
nat_melt$variable[nat_melt$variable %in% "target_2015"] <- "National 2015 Target (Standard Cases)"

act_comp$variable <- "Denver Metro (All Cases)"
...
ltbi_plot <- rbind(ltbi_comp[ , c("plot_qtr", "ltbi_drug", "per_complete")],
                   data.frame(plot_qtr = sort(unique(ltbi_comp[ , "plot_qtr"])),
                              ltbi_drug = "National 2015 Target for Contacts",
                              per_complete = 79)
)
