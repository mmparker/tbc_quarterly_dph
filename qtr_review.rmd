...
actives <- query_actives(start_date = get_quarter(Date = Sys.Date(), 
                                                offset = (4 * -10)
                                      )$start.date,
...
# Merge and calculate the annual case rates
actives_count$year <- substr(actives_count$plot_qtr, 1, 4)

annual_count <- ddply(actives_count, .var = "year", .fun = summarise, freq = sum(freq))

actives_rate <- merge(x = annual_count,
                      y = metro_pop,
                      by = "year",
                      all.x = TRUE)
...
actives_rate$rate_pht[actives_rate$year %in% substr(last.qtr$qtr, 1, 4)] <- 
    with(actives_rate[actives_rate$year %in% substr(last.qtr$qtr, 1, 4), ],
...
actives_print <- merge(x = actives_print,
                       y = actives_rate[ , c("year", "rate_pht")],
                       by.x = "yr_id",
                       by.y = "year",
                       all.x = TRUE)
...
plans <- query_tx_plans(start_date = get_quarter(Date = Sys.Date(),
...
dfprint(arrange(act_inc, plot_qtr, person_id))
...
plans_trim <- plans[!(plans$ltbi_drug %in% "INH/Rifapentine" & plans$plan_yr < 2012), ]
...
dfprint(arrange(ltbi_inc, plot_qtr, person_id))
