...
```{r visit_table, results='asis'}

# Cast visits wide
visits_wide <- dcast(visitagg, plot_qtr ~ location, value.var = "freq")

# Aggregate by year
visits_wide$year <- substr(visits_wide$plot_qtr, 1, 4)

# Pretty-print
names(visits_wide) <- c("Quarter", "Total Visits", "Clinic Visits", "Outreach Visits", "Year")

dfprint(visits_wide[ , c("Year", "Total Visits", "Clinic Visits", "Outreach Visits")])
